...
##########        Constatnts Declaration      ###########
const DRAG_LIM_MAX: Vector2 = Vector2(0, 60)
const DRAG_LIM_MIN: Vector2 = Vector2(-60, 0)
const IMPULSE_MULTI: float = 20.0
...
func _physics_process(delta):
	update(delta)
	# Updates the label using states in the list (found in State Init)
	label.text = "%s\n" % ANIMAL_STATE.keys()[_state]
	label.text += "%.1f,%.1f" % [_dragged_vector.x, _dragged_vector.y]

func get_impulse() -> Vector2:
	return _dragged_vector * -1 * IMPULSE_MULTI
...
func set_new_sate(new_state: ANIMAL_STATE) -> void:
	_state = new_state
...
func detect_release() -> bool:
	if _state == ANIMAL_STATE.DRAG:
		if Input.is_action_just_released("drag") == true:
			set_new_sate(ANIMAL_STATE.RELEASE)
			return true
	return false
	
func scale_arrow() -> void:
	var impulse_len = get_impulse().length()
	var impulse_percent = impulse_len / IMPULSE_MAX
	
	arrow.scale.x = (_arrow_scale_x * impulse_percent) + _arrow_scale_x
...
func _on_input_event(viewport, event, shape_idx):
	if _state == ANIMAL_STATE.READY and event.is_action_pressed("drag"):
		set_new_sate(ANIMAL_STATE.DRAG)
